...
Windows is a process running on your computer and your process needs to be linked.
Linking process starts from WinMain entry point.
...
Can check disassembly to see assembly conversion of code written in Visual Studio.
...
Modern ones are all little endians where lower order bytes are filled first and then higher order bytes.
For eg `unsigned int = 500` would be `256 + 244` or `00000001 11110100` which in memory is `244 and 1`.
That means reading the smaller subset (like 8 bytes out of 16 bytes) is easier with little endian.

Ps3 Xbox 360 and older were big endian, ps4 onwards is little.
Old formats might be in big endian > might need to convert into little endian to read.
...
Throughput or Bandwidth is the amount of operation that can be done between first round trip started until the first round trip is finished.
Or in peak traffic how many operations can I start in a second.
...
In production mode, compiler wont spend effort trying to do that.
